
python3.11 download.py dslim bert-base-NER
python3.11 download.py protectai deberta-v3-base-prompt-injection-v2
python3.11 download.py MoritzLaurer deberta-v3-base-zeroshot-v1.1-all-33
python3.11 download.py tomaarsen span-marker-bert-base-orgs
python3.11 download.py unitary unbiased-toxic-roberta
python3.11 download.py philomath-1209 programming-language-identification
python3.11 download.py madhurjindal autonlp-Gibberish-Detector-492513457
python3.11 download.py papluca xlm-roberta-base-language-detection
python3.11 download.py Isotonic deberta-v3-base_finetuned_ai4privacy_v2
exit
git lfs install
git clone git@hf.co:protectai/deberta-v3-base-prompt-injection-v2
git clone git@hf.co:MoritzLaurer/deberta-v3-base-zeroshot-v1.1-all-33
git clone git@hf.co:tomaarsen/span-marker-bert-base-orgs
git clone git@hf.co:unitary/unbiased-toxic-roberta
git clone git@hf.co:philomath-1209/programming-language-identification
git clone git@hf.co:madhurjindal/autonlp-Gibberish-Detector-492513457
git clone git@hf.co:papluca/xlm-roberta-base-language-detection
git clone git@hf.co:Isotonic/deberta-v3-base_finetuned_ai4privacy_v2
